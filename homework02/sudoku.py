...
def get_block(grid: tp.List[tp.List[str]], pos: tp.Tuple[int, int]) -> tp.List[str]:
    ...
    row = (pos[0] // 3)
    col = (pos[1] // 3)
    return [grid[row * 3 + i][col * 3 + j] for i in range(3) for j in range(3)]
...
def solve(grid: tp.List[tp.List[str]]) -> tp.Optional[tp.List[tp.List[str]]]:
    ...
    """
    empos = find_empty_positions(grid)
    if not empos:
        return grid
    row, col = empos
    for i in find_possible_values(grid, empos):
        grid[row][col] = i
        if solve(grid):
            return solve(grid)
        else:
